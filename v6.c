...
pthread_cond_t condBoxAssigned;
pthread_cond_t condBox[2];
pthread_cond_t condIsOut;
...
int main(int argc, char *argv[]){
	...
	pthread_cond_init(&condIsOut,NULL);
	...
	dinamic.sa_handler = assignRacers;
...
void assignRacers(){
	...
	printf("Introduce el número de corredores: ");
	scanf("%d",&newMaxRacers);
	printf("Introduce el número de boxes: ");
	scanf("%d",&newMaxBoxes);
	...
	boxTmp = (BoxParameters *)realloc(arrayBoxes,newMaxBoxes);
	free(boxesWaitList);
	free(arrayCars);
	free(arrayBoxes);
	if(listTmp==NULL||carsTmp==NULL||boxTmp==NULL){
	...
	for(;;){
		...
		if(params->racerPos!=-1){
			...
			pthread_cond_signal(&condBox[params->boxID]);
				while(params->carIsOut==0){
					pthread_cond_wait(&condIsOut,&mutexBox[params->boxID]);
				}
	...
	for(;;){
		...
		if(params->repared==1){
			...
			pthread_mutex_lock(&mutexBox[params->boxAssociated]);
				...
				pthread_mutex_lock(&mutexBoxes);
				arrayBoxes[params->boxAssociated].carIsOut=1;
				pthread_mutex_unlock(&mutexBoxes);
				pthread_cond_signal(&condIsOut);
				sprintf(msg, "Sale del box_%d",params->boxAssociated);
				writeLogMessage(racerNum,msg);
				aux=params->boxAssociated;
				params->boxAssociated=-1;
			pthread_mutex_unlock(&mutexBox[aux]);
